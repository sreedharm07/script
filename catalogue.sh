...
mongo --host mongodb.cloudev7.online </app/schema/catalogue.js  &>>/tmp/roboshop.log  &>>/tmp/roboshop.log

echo -e "\e[32m>>>>>>crestarting<<<<<<<<\e[0m"
systemctl daemon-reload  &>>/tmp/roboshop.log  &>>/tmp/roboshop.log
systemctl enable catalogue  &>>/tmp/roboshop.log  &>>/tmp/roboshop.log
systemctl start catalogue  &>>/tmp/roboshop.log  &>>/tmp/roboshop.log
